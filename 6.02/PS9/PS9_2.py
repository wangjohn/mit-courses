...
# This file is for a sliding window protocol that achieves reliable, in-order,
...
class ReliableSenderNode(Router):
    def __init__(self,location,qsize,address=None,window=1):
    ...
    def reliable_send(self, time):
        for seqnum, packet in list(self.outstanding_packets.iteritems()):
            if time - packet.start > self.timeout:
                self.retransmit_queue[seqnum] = packet
                del self.outstanding_packets[seqnum]
        if len(self.retransmit_queue) > 0:
            seqnum = self.retransmit_queue.keys()[0]
            packet = self.retransmit_queue[seqnum]
            packet.start = time
            self.outstanding_packets[seqnum] = packet
            del self.retransmit_queue[seqnum]
            self.send_pkt(time, packet.properties.get('seqnum'), time, 'red')
        elif len(self.outstanding_packets.keys()) < self.window:
            packet = self.send_pkt(time, self.seq_num, time, 'black')
            packet.start = time
            self.outstanding_packets[self.seq_num] = packet
            self.seq_num +=1

    # An ACK just arrived; process it.  Remember to call calc_timeout with the
    # appropriate information.
    def process_ack(self, time, acknum, pkt_timestamp):
        if acknum in self.outstanding_packets:
            del self.outstanding_packets[acknum]
            self.calc_timeout(time, pkt_timestamp)
    ...
    def receive(self,p,link,time):
        ...
        if self.network.verbose == True:
            print "t=%d sender %s received ACK %d" % (time, self.address, acknum)
        self.process_ack(time, acknum, pkt_timestamp)

# ReliableReceiverNode extends Router, implements receiver side of reliable
# transport protocol.
class ReliableReceiverNode(Router):
    ...
    def reset(self):
        ...
        ## Your code for initializing the receiver should go here (not in init).
        ## Initialize the receiver buffer, received sequence number, and any 
        ## other variables of your choice.
        self.rcv_seqnum = 0
        self.rcvbuf = []
        self.maxrcvbuf = 0
    ...
    def reliable_recv(self, sender, time, seqnum, pkt_timestamp):
        self.app_queue[seqnum] = time
        self.send_ack(sender, time, seqnum, pkt_timestamp)
        while self.app_seqnum+1 in self.app_queue:
            self.app_receive(self.app_seqnum+1, self.app_queue[self.app_seqnum+1])
            del self.app_queue[self.app_seqnum]
    ...
    def app_receive(self, seqnum, time):
        try:
            ...
            if self.network.verbose == True:
                print "t=%d receiver %s app_receive seqnum %d" % (time, self.address, seqnum)
...
#                    print "       max rcvbuf", self.maxrcvbuf
#                    self.maxrcvbuf = 0
...
class MyNetwork(RouterNetwork):
    ...
    def make_node(self,loc,address=None):
        if address == 'S':
            return ReliableSenderNode(loc,self.qsize,address,self.window)
        elif address == 'R':
            return ReliableReceiverNode(loc,self.qsize,address,self.window)
        else:
...
if __name__ == '__main__':
    ...
    parser.add_option("-w", "--window", type="int", dest="window", 
                      default=1, help="window size (default: 1)")
    ...
    # make a network
    net = MyNetwork(opt.simtime, NODES, LINKS, opt.window, opt.qsize, \
