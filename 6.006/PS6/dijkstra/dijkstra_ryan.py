...
class PathFinder(object):
    ...
    def dijkstra(self, weight, nodes, source, destination):
        ...
        for node in nodes:
            ...
            if node == source:
                nodedistance[node] = NodeDistancePair(node, 0)
                q.insert(nodedistance[node])
                
                
            else:
                nodedistance[node] = NodeDistancePair(node, sys.maxint)
                q.insert(nodedistance[node])
        ...
        while q.heap != [None]:
            ...
            if u.node ==destination:
                ...
                path.reverse()
                return (path, nodedistance[destination].distance)
            s.append(u)
            for vnode in u.node.adj:
                v = nodedistance[vnode]
                if v.distance > u.distance + weight(u.node, v.node):
                    v.distance = u.distance + weight(u.node, v.node)
                    predecessor[v.node] = u.node
                    q.decrease_key(v)
