...
class NodePriorityQueue(object):
    ...
    def _decrease_key(self, i, node):
        ...
        while i > 1:
            parent = i // 2
            if self[parent].distance > node.distance:
                self._swap(i, parent)
                i = parent
            else:
                break
    ...
    def _min_heapify(self, i):
        ...
        r = 2 * i + 1
        smallest = i
        if l <= len(self) and self[l].distance < self[i].distance:
            smallest = l
        if r <= len(self) and self[r].distance < self[smallest].distance:
            smallest = r
        if smallest != i:
    ...
    def check_ri(self):
        ...
        while i <= (len(heap) - 1) // 2:
            l = i * 2
            if heap[i].distance > heap[l].distance:
                raise ValueError('Left child is smaller than parent.')
            r = i * 2 + 1
            if r < len(heap) and heap[i].distance > heap[r].distance:
