...
class AugmentedPriorityQueue(object):
    """ Augmented version of the priority queue that allows you to augment the key. """
    def __init__(self):
    ...
    def insert(self, node)
        if node.distance not in self.nodes:
            self.nodes[node.distance] = [node]
        else:
            self.nodes[node.distance].append(node)
        self.heap.insert(node.distance)

    def extract_min(self):
        min_distance = self.heap.extract_min()
        min_node_list = self.nodes[min_distance]
        min_node = min_node_list[0]
        return min_node

    # Note this might break if there are multiple node.distances
    # in the current priority Queue because the distance will be
    # overwritten
    def decrease_key(self, node):
        self.heap.decrease_key(node.distance)
