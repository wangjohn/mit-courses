...
\newline
\newline
def \textbf{kthRoot}(A, K, N):
\par{} \hspace{0.5cm} minimum = \textbf{Zero}(N)
\par{} \hspace{0.5cm} maximum = \textbf{Word}($2^{(\textbf{Ceiling}(N/K) - 1)}$ )
\par{} \hspace{0.5cm} while True:
\par{} \hspace{1cm} middle = \textbf{Add}(minimum, maximum, N) $>>$ 1
\par{} \hspace{1cm} for i in range(K):
\par{} \hspace{1.5cm} middle = \textbf{Multiply}(middle, middle, N) 
\par{} \hspace{1cm} if \textbf{Cmp}(middle, A, N) == Greater:
\par{} \hspace{1.5cm} maximum = middle
\par{} \hspace{1cm} elif \textbf{Cmp}(middle, A, N) == Less:
\par{} \hspace{1.5cm} mimimum = middle
\par{} \hspace{1cm} else:
\par{} \hspace{1.5cm} break
\par{} \hspace{0.5cm} \textbf{return} middle
\newline
\newline
\newline
\textbf{Running Time}: The running time of this algorithm can be seen because the variables minimum and maximum are both N digit long variables. Next, we see that the \textbf{Add} and \textbf{Cmp} commands inside the while loop take $\Theta(N)$. The multiplication inside the for loop will take $\Theta(K N^{\log_2 3})$, because each loop multiply takes $\Theta(N^{\log_2 3})$ in the worst case. Moreover, $K$ in the worst case is equal to $N$, so each iteration of the while loop takes $\Theta(N^{1 + \log_2 3})$. Now, the while loop will go until middle and A are the same within $N$ digits of precision. This binary search will take $\Theta( \lg( 2^{N/K})) = \Theta( N / K) = \Theta(N)$. Thus, the entire algorithm is $\Theta(N^{2 + \log_2 3})$.   
...
\li        \Do
             $\id{digit} \gets \proc{Word}(A[i]) + \proc{Word}(B[j]) +
                               \proc{Word}(C[i + j - 1]) +
                               \proc{Word}(\id{carry})$
\li           $C[i + j - 1] \gets \proc{Msb}(\id{digit})$
\li           $\id{carry} \gets \proc{Lsb}(\id{digit})$
           \End
...
\li \Until $\id{S_i}[N] > 0$ or $\proc{Cmp}(S_i, A) \isequal \const{Greater}$
...
\li       \Do
            \If $E[i] \textrm{ \& } \id{mask} \isequal 0$
\li         \Then
...
\begin{enumerate}
  ...
  \item Test \texttt{4future\_1024} shows that RSA has fixed points.
