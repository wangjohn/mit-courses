...
def alpha_beta_search(board, depth,
    ...
    return get_next_moves(board, depth, eval_fn, -sys.maxint-1, sys.maxint, get_next_moves_fn, is_terminal_fn)
...
def alpha_beta_find_values(board, depth, eval_fn, old_alpha, old_beta, get_next_moves_fn=get_all_next_moves, is_terminal_fn=is_terminal):
    ...
    if is_terminal_fn(depth, board):
        return eval_fn(board)
    return get_next_moves(board, depth, eval_fn, alpha, beta, get_next_moves_fn, is_terminal_fn)


def get_next_moves(board, depth, eval_fn, alpha, beta, get_next_moves_fn, is_terminal_fn):
    best_value = None
    for move, new_board in get_next_moves_fn(board):
        (val, old_move, old_board) = alpha_beta_find_values(new_board, depth-1, eval_fn, alpha, beta, get_next_moves_fn, is_terminal_fn)
        if val > alpha[0]:
            alpha = (val, move, new_board)
        if val < beta[0]:
            beta = (val, move, new_board)
        if best_value == None or val > best_value[0]:
            best_value = (val, move, new_board)
        if alpha[0] >= beta[0]:
            return best_value
    return best_value 
