...
class FindUserSets:
    ...
    def format_both_ba_into_rows_meancentered(self, both_ba, output_rows):
        ...
        for user_account_id, activity_logs_list in both_ba.iteritems():
            for i in xrange(len(activity_logs_list[0])):
                ...
                j = binary_search_on_created_at(activity_logs_list[0], activity_log_before.created_at + datetime.timedelta(days=1), 0, len(activity_logs_list[0])-1)
            ...
            for i in xrange(len(activity_logs_list[1])):
                ...
                j = binary_search_on_created_at(activity_logs_list[1], activity_log_after.created_at + datetime.timedelta(days=1), 0, len(activity_logs_list[1])-1)
                output_rows.append(activity_log_after.convert_to_row() + [1, j-i])
        return output_rows
